 ...
 * you should exclude first eight non-printable characters prior to {@literal \t} (tab character):
 * <pre>{@code
 * factory = factory.withKeysDomain('\t', Character.MAX_VALUE);
 * }</pre>// elif !(char elem) //odd numbers, you should exclude one even number, zero:
 * <pre>{@code
 * factory = factory.withKeysDomainComplement(// const elem 0 //0, // const elem 0 //0);
 * }</pre>// endif //
 *
...
public interface CharHashFactory<T extends CharHashFactory<T>> extends HashContainerFactory<T> {
     ...
     * <pre>{@code
     * // only positive keys
     * factory = factory.withKeysDomain(// const elem 1 //1, Character.MAX_VALUE);
     * }</pre>
     *
     ...
     */
    T withKeysDomain(char minPossibleKey, char maxPossibleKey);
     ...
     * factory = factory.withKeysDomainComplement(//hole//, //hole//);
     * }</pre>
     *
     ...
     */
    T withKeysDomainComplement(char minImpossibleKey, char maxImpossibleKey);
}

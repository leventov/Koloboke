 ...
 *     {@link net.openhft.collect.set.hash.HashCharSet} <i>interface</i> extends
 *     {@link net.openhft.collect.set.CharSet}, which extends {@link java.util.Set} and
 *     {@link net.openhft.collect.CharCollection}, which extends {@link java.util.Collection}
 *     and {@link net.openhft.collect.Container}.
 *
 *     <p>Also,// if !(JDK8 jdk) // a complete set of functional interfaces is defined
 *     in the {@code net.openhft.function} package, it is needed because the library backports many
 *     methods from Java 8 Collections API (see <a href="#api-additions">API additions</a>), which
 *     employ these interfaces.// elif JDK8 jdk // the {@code net.openhft.function} package
 *     polyfills {@link java.util.function} functional interface set with the rest specializations
 *     ({@link java.util.function} package defines only some specializations for {@code int},
 *     {@code long} and {@code double} types.)// endif //</li>
 *
 ...
 *     a {@link net.openhft.collect.hash.HashContainerFactory} to configure all factories which
 *     produce hash sets and maps in the application.
 *
 ...
 *     {@link net.openhft.collect.map.hash.HashIntShortMaps} define static factory methods which
 *     return {@link net.openhft.collect.map.hash.HashIntShortMap} instances, delegating to the
 *     default {@link net.openhft.collect.map.hash.HashIntShortMapFactory} implementation.</li>
 * </ol>
 *
 ...
 * permits faster implementation of {@linkplain net.openhft.collect.hash.HashContainer hash
 ...
 * which accept closures// endif //, the library supplies {@linkplain net.openhft.collect.Cursor
 * cursors} for every container// if !(JDK8 jdk) // and {@code forEach()}-like methods which accept
 * closures// endif //.
 *
 * <table BORDER CELLPADDING=3 CELLSPACING=1>
 ...
 * difference with the most widely used implementation from JCF of the same parental interface. For
 * example, {@link net.openhft.collect.set.hash.HashCharSet} extends
 ...
 *     with {@code Object} keys, e. g. {@link net.openhft.collect.map.hash.HashObjDoubleMap}, don't
 *     support {@code null} key by default, you should configure
 *     {@code factory.}{@link net.openhft.collect.hash.ObjHashFactory#withNullKeyAllowed(boolean
 *     ) withNullKeyAllowed(true)}.
 *     </li>
 ...
 *     <a href="https://github.com/OpenHFT/hftc/issues/14">the issue</a>.</li>
 *     <li>Collections don't implement {@link java.io.Serializable} yet. To be fixed, see
 *     <a href="https://github.com/OpenHFT/hftc/issues/15">the issue</a>.</li>
 ...
 *     On attempt of insertion the last keys {@link net.openhft.collect.hash.HashOverflowException}
 *     is thrown.</li>
 ...
 *     {@link net.openhft.collect.CharCollection} extends {@link java.util.Collection},
 *     {@link net.openhft.collect.map.IntFloatMap} extends {@link java.util.Map}. There are also
 *     classes with {@code Obj-} prefix, they bring <a href="#api-additions">API additions</a> to
 *     collections of objects, if there are no additions for the class or interface, {@code Obj-}
 *     "specializations" are present anyway, for global API symmetry.</li>
 ...
 *     {@link net.openhft.collect.ByteCollection#removeByte(byte)} is a specialized version of
 *     {@link java.util.Collection#remove(Object)}, but have a different name (the same for
 *     {@link net.openhft.collect.LongCollection}, {@link net.openhft.collect.FloatCollection}, etc.
 *     for symmetry. This is because {@code remove(int)}
 *     in {@link net.openhft.collect.IntCollection} will conflict with
 ...
 *         <li>{@link net.openhft.collect.map.ObjCharMap#getChar(Object)}</li>
 *         <li>{@link net.openhft.collect.map.ObjCharMap#removeAsChar(Object)}</li>
 *         <li>{@link net.openhft.collect.CharIterator#nextChar()}</li>
 *         <li>{@link net.openhft.function.ToCharFunction#applyAsChar(Object)}</li>
 *     </ul>
 *     </li>
 *     <li>Method {@link net.openhft.collect.DoubleCollection#toDoubleArray()}, and others similar,
 *     is exceptional from those rules and have special name.</li>
 * </ol>
 *
 ...
 * factories in the library allow to configure {@linkplain net.openhft.collect.Equivalence
 ...
 * the documentation to {@link net.openhft.collect.ObjCollection#equivalence()},
 * {@link net.openhft.collect.map.ObjObjMap#keyEquivalence()} and
 * {@link net.openhft.collect.map.ObjObjMap#valueEquivalence()} methods for more information.
 *
 * <h3>Functional additions to {@link java.util.Collection} interface:</h3>
 * <ul>
 *     // if !(JDK8 jdk) //
 *     <li>{@link net.openhft.collect.ObjCollection#forEach(net.openhft.function.Consumer)} simply
 *     performs the given action for each element of the collection. This method is backported from
 *     Java 8 {@code Collection} API.</li>
 *     <li>{@link net.openhft.collect.ObjCollection#removeIf(net.openhft.function.Predicate)}
 ...
 *     // endif //
 *     <li>{@link net.openhft.collect.ObjCollection#forEachWhile(net.openhft.function.Predicate)}
 *     performs the given action for each element of the collection, while it returns {@code true}.
 *     </li>
 *     <li>Containers in the library support {@link net.openhft.collect.Cursor} iteration:
 *     {@link net.openhft.collect.ObjCollection#cursor()}. See also
 ...
 * primitive specializations: {@link net.openhft.collect.ByteCollection},
 * {@link net.openhft.collect.CharCollection}, etc.
 *
 ...
 *         <ul>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#getOrDefault(Object, Object)}</li>
 *             <li>{@link
 *             net.openhft.collect.map.ObjObjMap#forEach(net.openhft.function.BiConsumer)}</li>
 *             <li>{@link
 *             net.openhft.collect.map.ObjObjMap#replaceAll(net.openhft.function.BiFunction)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#putIfAbsent(Object, Object)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#remove(Object, Object)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#replace(Object, Object, Object)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#replace(Object, Object)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#computeIfAbsent(Object,
 *             net.openhft.function.Function)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#computeIfPresent(Object,
 *             net.openhft.function.BiFunction)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#compute(Object,
 *             net.openhft.function.BiFunction)}</li>
 *             <li>{@link net.openhft.collect.map.ObjObjMap#merge(Object, Object,
 *             net.openhft.function.BiFunction)}</li>
 *         </ul>
 *     </li>
 *     <li>
 ...
 *         <ul>
 *     // endif //
 *             <li>{@link net.openhft.collect.map.ObjObjMap#cursor()} &mdash; cursor iteration over
 *             maps.</li>
 *             <li>{@link
 *             net.openhft.collect.map.ObjObjMap#forEachWhile(net.openhft.function.BiPredicate)}
 *             performs the given action for each entry of the map, while it returns {@code true}.
 *             </li>
 *             <li>{@link
 *             net.openhft.collect.map.ObjObjMap#removeIf(net.openhft.function.BiPredicate)} removes
 *             all of the entries of this map that satisfy the given predicate.</li>
 *             <li>{@link net.openhft.collect.map.ObjIntMap#addValue(Object, int)} and
 *             {@link net.openhft.collect.map.ObjIntMap#addValue(Object, int, int)} add the given
 *             value to the value associated to the given key. These methods are present in the map
 *             specializations with primitive value.</li>
 *     // if !(JDK8 jdk) //
 *         </ul>
 *     </li>
 *     // endif //
 ...
 * to {@link net.openhft.collect.hash.HashContainer} and {@link net.openhft.collect.hash.HashConfig}
